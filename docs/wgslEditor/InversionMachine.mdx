...
export default function WgslAudioEditorWithInputs() {
  ...
  useEffect(() => {
    if (engine && engine.updateAudioParams) {
      engine.updateAudioParams(new Float32Array([bouncePitch, fractalIterations, noiseTime, squelchMod, volume, squelch, squelchSpeed, bounceTime, fractalGain, bounceGain, squelchGain, noiseGain, alarmTime, alarmNoise, alarmGain]));
    }
  }, [engine, bouncePitch, fractalIterations, noiseTime, squelchMod, volume, squelch, squelchSpeed, bounceTime, fractalGain, bounceGain, squelchGain, noiseGain, alarmTime, alarmNoise, alarmGain]);
