...
async function downloadTexture(device, texture, filename = "image.png") {
  ...
  commandEncoder.copyTextureToBuffer({texture: texture, origin: {x: 0, y: 0}}, {
    buffer: copiedBuffer,
...
async function downloadStencilTexture(device, texture, filename = "image.png") {
  ...
  commandEncoder.copyTextureToBuffer({
    ...
    origin: {x: 0, y: 0},
    aspect: "stencil-only"
  }, {buffer: copiedBuffer, bytesPerRow: bufferWidth}, {width: texture.width, height: texture.height});
  device.queue.submit([commandEncoder.finish()]);
