...
importScripts("/scripts/OBJFile.js");
...
class ObjectModel {
  ...
  async setup(device, code, modelViewMatrixUniformBuffer, projectionMatrixUniformBuffer, normalMatrixUniformBuffer) {
    ...
    let {positionBuffer, normalBuffer, indexBuffer, indexSize} = await loadObj(device, '/obj/knot.obj');
...
async function run(canvas, code) {
  ...
  await objectModel.setup(device, code, modelViewMatrixUniformBuffer, projectionMatrixUniformBuffer, normalMatrixUniformBuffer);
  ...
  async function render() {
    ...
    let modelViewMatrix = glMatrix.mat4.lookAt(
      ...
      glMatrix.vec3.fromValues(Math.cos(angle*angle) * 5.0, Math.sin(angle*angle) * 5.0, 5),
    ...
    objectModel.encode(passEncoder);
    passEncoder.end();
