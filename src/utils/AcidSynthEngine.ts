...
export default class AcidSynthEngine {
  ...
  public async initGPU({code, entryPoint, workgroupSize}) {
    ...
    this.audioParamBuffer = this.device.createBuffer({
      size: Float32Array.BYTES_PER_ELEMENT * 15, //paramArray.length
  ...
  public updateAudioParams(paramArray: Float32Array) {
    this.device.queue.writeBuffer(this.audioParamBuffer, 0, new Float32Array(paramArray));
  }
